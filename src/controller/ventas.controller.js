...
export const createVentas = async (req, res) => {
        ...
        const { codigo_producto, nombre_cliente, telefono_cliente, fecha_venta, cantidad_vendida, total_venta } = req.body; // Cambié "Nombre_cliente" a "nombre_cliente"
        
        try {
            const [rows] = await pool.query('INSERT INTO ventas (codigo_producto, Nombre_cliente, telefono_cliente, fecha_venta, cantidad_vendida, total_venta) VALUES (?,?,?,?,?,?)', [codigo_producto, nombre_cliente, telefono_cliente, fecha_venta, cantidad_vendida, total_venta]); // Cambié "Nombre_cliente" a "nombre_cliente"
            
            console.log(rows);
            
            res.send({
                codigo_producto, nombre_cliente, telefono_cliente, fecha_venta, cantidad_vendida, total_venta // Cambié "Nombre_cliente" a "nombre_cliente"
            });
        } catch (error) {
            return res.status(500).json({ message: 'Ha ocurrido un error' });
        }
